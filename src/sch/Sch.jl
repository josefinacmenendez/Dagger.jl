...
function do_task(to_proc, comm)
    ...
    lock(TASK_SYNC) do
        ...
        while true
            ...
            if est_alloc_util > storage_cap
                debug_storage("WARN: Estimated utilization above storage capacity on $to_storage_name, proceeding")
                break
            end
            if est_alloc_util + real_alloc_util > storage_cap
                if externally_varying(to_storage)
                    debug_storage("WARN: Insufficient space and allocation behavior is externally varying on $to_storage_name, proceeding")
                    break
                end
                if hang_ctr > 100
                    debug_storage("WARN: Insufficient space and hang detected on $to_storage_name, proceeding")
                    break
                end
                if length(TASKS_RUNNING) <= 2 # This task + eager submission task
                    debug_storage("WARN: Insufficient space and no other running tasks on $to_storage_name, proceeding")
                    break
                end
    ...
    debug_storage("Releasing $to_storage_name")
    metadata = (
